...
interface CronJobServiceDeps {
  ...
  /** Called when a run finishes (success, error, or skipped). */
  onRunCompleted?: (run: ScheduledTaskRunWithName) => void;
}
...
export class CronJobService {
  ...
  private readonly onRunCompleted?: (run: ScheduledTaskRunWithName) => void;
  ...
  constructor(deps: CronJobServiceDeps) {
    ...
    this.onRunCompleted = deps.onRunCompleted;
  }
  ...
  private async pollOnce(): Promise<void> {
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        const previousRunAtMs = this.lastKnownRunAtMs.get(job.id);
        if (previousRunAtMs !== undefined && lastRunAtMs > previousRunAtMs) {
          try {
  ...
  private emitRunUpdate(run: ScheduledTaskRunWithName): void {
    ...
    // Notify the host when a run finishes (not while still running).
    if (run.status !== TaskStatus.Running && this.onRunCompleted) {
      try {
        this.onRunCompleted(run);
      } catch {
        // Notification failures must not break polling.
      }
    }
  }
