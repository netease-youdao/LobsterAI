...
export class SqliteStore {
  ...
  private initializeTables(basePath: string) {
    ...
    // Migration: Add agent_id and color columns to cowork_sessions
    try {
      ...
      if (!sessionColNames.includes('color')) {
        this.db.exec('ALTER TABLE cowork_sessions ADD COLUMN color TEXT;');
      }
    } catch {
      // Column already exists or migration not needed.
    }
