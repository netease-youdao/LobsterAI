...
import { app, BrowserWindow, dialog, ipcMain, Menu, nativeImage, nativeTheme, net, powerMonitor, powerSaveBlocker,protocol, session, shell, systemPreferences } from 'electron';
import fs from 'fs';
import os from 'os';
import path from 'path';

import { DeliveryChannel } from '../scheduledTask/constants';
...
import { getNativeNotificationService } from './nativeNotificationService';
...
const bindCoworkRuntimeForwarder = (): void => {
  ...
  runtime.on('complete', (sessionId: string, claudeSessionId: string | null) => {
    ...
    // Native notification for session completion.
    // For scheduled-task sessions: check delivery config — only notify when channel='local'.
    // For regular cowork sessions: always notify.
    try {
      if (openClawRuntimeAdapter?.isScheduledTaskSession(sessionId)) {
        const jobId = openClawRuntimeAdapter.getCronJobId(sessionId);
        if (jobId) {
          const delivery = getCronJobService()?.getDeliverySync(jobId);
          if (delivery?.channel === DeliveryChannel.Local) {
            const jobName = getCronJobService()?.getJobNameSync(jobId);
            if (jobName) {
              getNativeNotificationService().notifyScheduledTaskComplete(jobName);
            }
          }
        }
      } else {
        ...
        if (title) {
          getNativeNotificationService().notifyCoworkComplete(title);
        }
      }
    } catch {
