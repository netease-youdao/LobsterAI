...
export class CoworkRunner extends EventEmitter {
  ...
  async startSession(
    ...
    try {
      ...
      await this.runClaudeCode(activeSession, effectivePrompt, sessionCwd, effectiveSystemPrompt, options.imageAttachments, apiConfigOverride);
    } catch (error) {
  ...
  async continueSession(
    ...
    try {
      const promptPrefix = this.buildPromptPrefix();
      const effectivePrompt = promptPrefix ? `${promptPrefix}\n\n---\n\n${prompt}` : prompt;
      await this.runClaudeCode(activeSession, effectivePrompt, sessionCwd, effectiveSystemPrompt, options.imageAttachments, options.apiConfigOverride);
    } catch (error) {
