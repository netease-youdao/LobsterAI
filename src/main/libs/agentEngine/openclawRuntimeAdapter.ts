...
export class OpenClawRuntimeAdapter extends EventEmitter implements CoworkRuntime {
   ...
   * Used by the cowork completion handler to route notifications correctly.
   */
  isScheduledTaskSession(sessionId: string): boolean {
    return this.channelSessionSync?.isCronSessionId(sessionId) ?? false;
  }

  /**
   * Returns the gateway jobId for a cron session, or null if not a cron session.
   * Used to look up the task's delivery config when the session completes.
   */
  getCronJobId(sessionId: string): string | null {
    return this.channelSessionSync?.getCronJobId(sessionId) ?? null;
  }
