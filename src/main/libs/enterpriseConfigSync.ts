...
export function mergeOpenClawConfigs(
  ...
  if (isRecord(runtimeConfig.agents)
    && runtimeConfig.agents.ownership === OpenClawAgentOwnership.Explicit
    && Array.isArray(enterpriseAgents?.list)) {
    // Legacy enterprise overlays must not reintroduce list/default markers
    // into the keyed roster generated by LobsterAI.
    const { list, ...agentConfig } = enterpriseAgents;
    const entries: Record<string, unknown> = {};
    for (const item of list) {
      if (!isRecord(item) || typeof item.id !== 'string' || !item.id.trim()
        || Object.hasOwn(entries, item.id)
        || (item.default !== undefined && typeof item.default !== 'boolean')) {
        throw new Error('[Enterprise] Invalid legacy agent roster in OpenClaw config');
      }
      const { id, default: _default, ...entry } = item;
      Object.defineProperty(entries, id, { value: entry, enumerable: true });
    }
    normalizedEnterpriseConfig.agents = {
      ...agentConfig,
      entries: { ...entries, ...(isRecord(agentConfig.entries) ? agentConfig.entries : {}) },
    };
  }
