...
export function isCronSessionKey(sessionKey: string): boolean {
  return isOpenClawCronSessionKey(sessionKey);
}
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateCronSession(sessionKey: string): string | null {
    ...
    const jobId = cronKey.scheduledTaskId;
    ...
    if (persistedSessionId) {
      ...
      if (persistedSession) {
        ...
        console.debug(
          '[ChannelSessionSync] reusing persisted cron session:',
          { jobId, agentId, sessionId: persistedSessionId },
        );
