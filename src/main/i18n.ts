...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    sessionDeletionStopUnconfirmed: '尚未确认任务已停止，暂未删除。请稍后重试。',
    sessionDeletionTargetChanged: '任务已变化，请刷新后重新确认删除。',
    remoteSessionRecoveryRequired: '这次任务的执行或模型设置尚未确认，请稍后重试或新建任务继续。模型更新后出现问题时，可在 Agent 引擎设置中重启引擎后重试；旧版本遗留记录可能仍无法自动恢复。原任务不会自动重新执行。',
  ...
  en: {
    ...
    sessionDeletionStopUnconfirmed: 'The task has not been confirmed stopped and was not deleted. Try again shortly.',
    sessionDeletionTargetChanged: 'The task changed. Refresh and confirm deletion again.',
    remoteSessionRecoveryRequired: 'The task execution or model settings are not yet confirmed. Retry or continue in a new task. For a model update, restart the engine in Agent Engine settings and retry; records from older versions may still require a new task. The original task will not be replayed.',
