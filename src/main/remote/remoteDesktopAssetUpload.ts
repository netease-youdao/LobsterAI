...
export async function uploadDesktopMessageAsset(job: DesktopMessageAssetJob, deps: DesktopMessageAssetUploadDependencies): Promise<RemoteInputAsset> {
  ...
  const request = async (path: string, init: RequestInit): Promise<Record<string, unknown>> => {
    ...
    if (!response.ok || envelope.code !== 0 || !envelope.data) fail('ASSET_UPLOAD_FAILED');
  ...
  if (asset.status !== 'ready') {
    ...
    const completed = new Set(asset.completedParts as unknown[]);
