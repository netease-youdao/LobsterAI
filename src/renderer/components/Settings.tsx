...
const Settings: React.FC<SettingsProps> = ({
  ...
  // Track whether the Settings panel is still mounted, to guard async callbacks
  const isMountedRef = useRef(true);
  const copilotAuthStatusRef = useRef(copilotAuthStatus);

  // Keep copilotAuthStatusRef in sync so the cleanup useEffect can read the
  // latest status without depending on the reactive state value.
  useEffect(() => {
    copilotAuthStatusRef.current = copilotAuthStatus;
  }, [copilotAuthStatus]);

  // On unmount: mark as unmounted and cancel any in-progress OAuth polling so
  // that the background IPC call is aborted and the token result (if it arrives
  // in a race) is silently discarded.
  useEffect(() => {
    return () => {
      isMountedRef.current = false;
      if (
        copilotAuthStatusRef.current === 'polling' ||
        copilotAuthStatusRef.current === 'awaiting_user'
      ) {
        window.electron.githubCopilot.cancelPolling().catch(() => {
  ...
  const handleCopilotSignIn = async () => {
    try {
      ...
      // If the panel was closed while polling, discard the result — the cleanup
      // useEffect has already called cancelPolling(), so this branch is only
      // reachable in a rare race between the response and the abort signal.
