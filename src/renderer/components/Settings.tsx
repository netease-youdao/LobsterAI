...
const Settings: React.FC<SettingsProps> = ({
  ...
  // Trigger model configuration onboarding tour when model tab is active for the first time
  useEffect(() => {
    if (activeTab !== 'model') return;
    if (!onboardingService.isReady() || onboardingService.isTourCompleted('model-config')) return;
    const timer = window.setTimeout(() => {
      onboardingService.startModelConfigTour();
    }, 400);
      ...
      case 'model':
        return (
          <div className="flex h-full">
            ...
            <div
              data-onboarding="provider-list"
              className="w-2/5 border-r border-border pr-3 space-y-1.5 overflow-y-auto"
            >
            ...
            <div
              data-onboarding="provider-settings"
              className="w-3/5 pl-4 pr-2 space-y-4 overflow-y-auto [scrollbar-gutter:stable]"
            >
              ...
              <div className="flex items-center justify-center text-sm text-secondary">
                ...
                <button
                  ...
                  onClick={async e => {
                    ...
                    setNoticeMessage(i18nService.t('onboardingResetDone'));
                  }}
                  className="bg-transparent border-none appearance-none px-1.5 py-0.5 -mx-1.5 -my-0.5 rounded-md cursor-pointer hover:text-primary dark:hover:text-primary transition-colors"
                >
                  {i18nService.t('onboardingResetTour')}
                </button>
              </div>
          ...
          <nav className="flex flex-col gap-0.5 px-3 pb-4">
            {sidebarTabs.map(tab => (
              <button
                ...
                data-onboarding={tab.key === 'model' ? 'settings-model-tab' : undefined}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="flex justify-end space-x-4 p-4 border-border border-t bg-background shrink-0">
              ...
              <button
                type="submit"
                disabled={isSaving}
                data-onboarding="settings-save-btn"
