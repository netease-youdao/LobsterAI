...
const AgentSettingsPanel: React.FC<AgentSettingsPanelProps> = ({ agentId, onClose, onSwitchAgent }) => {
  ...
  const [initialValues, setInitialValues] = useState({
    name: '', description: '', systemPrompt: '', identity: '', icon: '', skillIds: [] as string[],
  });
    ...
    window.electron?.agents?.get(agentId).then((a) => {
      if (a) {
        ...
        setInitialValues({
          ...
          identity: a.identity, icon: a.icon, skillIds: a.skillIds ?? [],
        });
      }
    });
  ...
  const isDirty = name !== initialValues.name || description !== initialValues.description ||
    systemPrompt !== initialValues.systemPrompt || identity !== initialValues.identity ||
    icon !== initialValues.icon ||
    JSON.stringify([...skillIds].sort()) !== JSON.stringify([...initialValues.skillIds].sort()) ||
    boundKeys.size !== initialBoundKeys.size || [...boundKeys].some((k) => !initialBoundKeys.has(k));
        ...
        {showDiscardConfirm && (
          <Modal onClose={() => setShowDiscardConfirm(false)} className="w-full max-w-sm mx-4 bg-surface rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="flex items-center justify-end gap-3 px-5 py-4 border-t border-border">
              <button onClick={() => setShowDiscardConfirm(false)} className="px-4 py-2 text-sm font-medium rounded-lg text-secondary hover:bg-surface-raised transition-colors">
                {i18nService.t('cancel') || 'Cancel'}
              </button>
              <button onClick={handleConfirmDiscard} className="px-4 py-2 text-sm font-medium rounded-lg bg-red-500 hover:bg-red-600 text-white transition-colors">
