...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    modelFailoverLabel: '备用模型',
    modelFailoverHint: '主模型请求失败时自动切换到备用模型（限速、服务器错误、网络异常等）',
    modelFailoverNone: '不使用备用模型',
    modelFailoverTriggered: '主模型不可用，已自动切换到备用模型',
  ...
  en: {
    ...
    modelFailoverLabel: 'Fallback Model',
    modelFailoverHint: 'Automatically switch to the fallback model when the primary model fails (rate limit, server error, network issues, etc.)',
    modelFailoverNone: 'No fallback model',
    modelFailoverTriggered: 'Primary model unavailable, automatically switched to fallback model',
