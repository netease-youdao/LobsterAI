...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    mcpUrlRequired: 'SSE/Streamable HTTP 类型需要填写 URL',
    mcpNameExists: '服务名称已存在',
    mcpTransportStdio: '标准输入输出 (stdio)',
    mcpTransportSse: '服务器推送事件 (SSE)',
    mcpTransportHttp: '可流式 HTTP (Streamable HTTP)',
    mcpTransportStdioHint: '本地启动一个 MCP 进程，通过标准输入输出通信。',
    mcpTransportSseHint: '连接传统 SSE MCP 端点。仅在服务明确要求 SSE transport 时选择这个类型。',
    mcpTransportHttpHint: '连接 streamable_http MCP 端点。大多数远程 HTTP MCP 服务应选择这个类型。',
    addKeyValue: '添加',
    saveMcpServer: '保存',
    // MCP 市场 & 注册表
    mcpInstalled: '已安装',
  ...
  en: {
    ...
    mcpUrlRequired: 'URL is required for SSE/Streamable HTTP transport',
    ...
    mcpTransportStdioHint: 'Launch a local MCP process and communicate over standard input/output.',
    mcpTransportSseHint: 'Connect to a legacy SSE MCP endpoint. Use this only when the server explicitly requires SSE transport.',
    mcpTransportHttpHint: 'Connect to a streamable_http MCP endpoint. Most remote HTTP MCP servers should use this transport.',
    addKeyValue: 'Add',
    saveMcpServer: 'Save',
