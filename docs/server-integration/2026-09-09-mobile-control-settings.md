...
2026-09-22 同步设计更新（代码已实现，未部署）：[当前生效服务同步方案](../../specs/bugfixes/remote-sync-target/2026-09-22-effective-server-sync-design.md)规定目标核对、首次历史同步、协议不足和真实冲突的处理。正常域名或客户端模式变化不要求用户选择“环境”；在线状态与当前绑定的同步状态仍分开展示，旧回包和旧服务 ACK 不能让新服务显示已同步。下文为既有设置交互记录。
