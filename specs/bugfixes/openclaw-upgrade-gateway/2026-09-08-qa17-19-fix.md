# OpenClaw 2026.8.1 升级：QA 第 17–19 行修复及验收

本次改动位于 `fix/openclaw-upgrade-migration-restart`，基于包体积优化提交
`aa9fca23609e295ddd25e9fb2503c41196291d48`，不新增 OpenClaw 源码 patch。
本说明记录修复及本地隔离验证结果；macOS 原机升级和真实 QQ 收发仍待 QA 验收。

## 第 17 行：Agent 配置拒绝后重启

QA 日志中的直接失败是 `agents.ownership=explicit` 与旧的 `default=true`
...
首先，OpenClaw 2026.8.1 内置的 QQ 官方配置合同针对
...
其次，在独立 Windows 数据目录中，**输入已通过运行时完整校验的 QQ 配置后，
通用 `doctor --fix` 仍会生成不合法的 QQ 配置并退出 1**，会话迁移未完成。
因此启动时改用上游公开的专用入口：
...
尚未确认 QA 的 `openclaw.json` 文件锁由哪个进程持有，也未确认第 19 行最初
剩余的那一个 store 是哪个文件。新增只读诊断：
...
测试使用独立目录和假凭据，没有连接真实 QQ、调用模型或启动用户已登录的应用。

- 首轮相关单测：7 个文件、141 个用例通过。
- 包装、SDK bridge、网关恢复、内存及企业同步回归：9 个文件、117 个用例通过
  （与首轮存在重叠，不累计为总数）。
- 调整为专用迁移入口并补企业兼容后的相关复跑：4 个文件、50 个用例通过。
...
- 使用打包 Electron 可执行文件运行官方迁移，主 Agent、worker 的会话导入成功；
  ...
  发送旧 default 标记被拒绝，没有安排兜底重启，网关仍健康。测试进程已退出。
- 额外验证共享 `state/sessions/sessions.json` 和已不在配置中的 retired Agent
...
本地运行报告位于 `D:/tmp/lobster-gateway-fix-20260908/`，成功的主流程报告位于
其 `packaging-fixture/probe-final/` 子目录。测试 runtime 的 `dist`/`node_modules`
为只读复用目录链接，不能对这个测试目录执行递归裁剪或跟随链接删除。

## QA 验收

必须重新构建 OpenClaw runtime/插件并打包，不能仅重编译 Electron 后搭配旧 QQ
插件目录。优先在 QA 原 macOS 升级数据的备份副本上验证：
...
4. 第 18 行若再次出现文件锁超时，收集同次启动的 main 和 gateway 日志，保留
   当时的锁文件及进程信息；第 19 行若有存储残留，按新日志定位具体路径及原因。
