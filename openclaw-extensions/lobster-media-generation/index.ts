...
const sanitizeArgsForLog = (args: Record<string, unknown>): Record<string, unknown> => {
  ...
  return {
    ...
    audioCount: Array.isArray(args.audios) ? args.audios.length : undefined,
...
const VideoGenerateSchema = Type.Object({
  ...
  audios: Type.Optional(Type.Array(Type.String(), { description: 'Reference audio absolute file paths, URLs, or WAV/MP3 data URLs for multimodal reference video generation.' })),
  audioRoles: Type.Optional(Type.Array(Type.String(), { description: 'Role for each audio input: "reference_audio".' })),
  referenceAudios: Type.Optional(Type.Array(Type.String(), { description: 'Reference audio absolute file paths, URLs, or data URLs. Alias of audios.' })),
  ...
  resolution: Type.Optional(Type.String({ description: 'Resolution: "480p", "720p", "768P", "1080p", or "2K". Valid values depend on model.' })),
  durationSeconds: Type.Optional(Type.Number({ description: 'Video duration in seconds. Valid range depends on model (e.g. MiniMax-H3: 4-15, MiniMax Hailuo: 6 or 10). Use -1 only for models that support auto duration. Use action="list" to check.', minimum: -1, maximum: 60 })),
    ...
    api.registerTool((ctx) => {
      ...
      return {
        ...
        description: [
          ...
          'Step 1: If the system prompt locks a user-selected model, do not call action="list" to choose or switch models. Otherwise call action="list" to inspect available models and supported parameters.',
          'Step 2: Call with action="generate" using the locked user-selected model when present, otherwise the model chosen from the list, plus valid parameters. Returns a taskId.',
          ...
          'If generation fails or the status result reports failure, stop and report the reason. Never retry with or automatically fall back to another model. If the selected model is unavailable, ask the user to manually change the LobsterAI model picker.',
